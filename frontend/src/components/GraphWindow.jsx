...
import { Box, Typography, Alert, CircularProgress, Grid2 } from '@mui/material';
...
const SinglePlot = ({ plotData }) => {
  ...
  const { traces, layout } = useMemo(() => {
    ...
    const axisCommon = {
      ...
      titlefont: { size: 12, color: textColor },
      ticks: 'outside',
      ticklen: 4,
      tickwidth: 1,
      showline: true,
    ...
    const layout = {
      ...
      title: {
          text: plotData.title,
          font: { size: 13, weight: 600, color: textColor }
      },
      xaxis: { ...axisCommon, title: { text: plotData.xlabel, font: { size: 12, color: textColor } } },
      yaxis: { ...axisCommon, title: { text: plotData.ylabel, font: { size: 12, color: textColor } } },
      font: { family: "'Roboto', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", size: 11, color: textColor },
      margin: { l: 60, r: 20, b: 50, t: 36 },
...
const GraphWindow = memo(({ plotDataUrl, isPlotReady, plotError, liveGraphData, isSimulating }) => {
  ...
  // completed data takes priority; fall back to live streaming data
  const displayData = data ?? liveGraphData;
